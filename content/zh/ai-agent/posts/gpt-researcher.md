...
### **2026 更新：从研究脚本演进为可嵌入的 Deep Research 基础设施**

截至 2026 年 7 月，GPT Researcher 已经不只是一个命令行研究代理。官方站点强调它可以生成带引用的多源深度报告，支持本地文档混合研究、多种 LLM、多种搜索引擎、多 Agent 协作，以及 PDF、Docx、Markdown、JSON、CSV 等导出格式。更值得关注的是，它提供了 MCP Server，把 deep_research、quick_search、write_report、get_research_sources、get_research_context 等能力暴露给其他 AI 助手调用。

另一个新信号是评估体系。官方页面引用了 2025 年 Carnegie Mellon DeepResearchGym 的结果，称 GPT Researcher 在 citation quality、report quality 和 coverage 上表现领先。这个结果不能替代你自己的业务评测，但它说明 Deep Research 类系统的竞争焦点正在从“能联网搜索”转向“引用质量、覆盖率、可复现评估和工作流可控性”。
...
3. GPT Researcher MCP Server - Getting Started, accessed on July 11, 2026, [https://docs.gptr.dev/docs/gpt-researcher/mcp-server/getting-started](https://docs.gptr.dev/docs/gpt-researcher/mcp-server/getting-started)
4. GPT Researcher LangGraph Multi-Agent Docs, accessed on July 11, 2026, [https://docs.gptr.dev/docs/gpt-researcher/multi_agents/langgraph](https://docs.gptr.dev/docs/gpt-researcher/multi_agents/langgraph)
