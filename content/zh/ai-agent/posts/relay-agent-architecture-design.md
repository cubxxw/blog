...
### Coordinator：编排所有 Agent 的 Agent
...
每层对应不同的场景，走哪一层由任务的推理复杂度和调用频率决定，并非「随机选」。
...
**关键设计**：失败时不做静默降级（返回原始内容），改为**明确抛出异常 + 写入 audit 日志**。这让工程团队可以追踪虚构防卫的触发率，持续改进 Prompt。
...
Relay 的 API 层基于 Hono + Bun，但更值得关注的是**中间件的组合方式**，而非框架选择。
...
### 方案 B+：Playwright MCP Chrome Extension
...
每增加一个用户，面试题库就变得更丰富。这是经典的网络效应——用于知识积累，而非社交。
...
Agent 工程目前最稀缺的是**生产工程的积累**，而非智能：如何防失控、如何控成本、如何在 AI 犯错时有兜底、如何让用户对系统保持信任。
