...
tldr:
  ...
  - "自动提取已改为单次 ADD-only；检索以语义候选为基础，再按可用情况融合 BM25 与实体信号。旧版外部 graph_store、Neo4j 和独立图检索路径已从 OSS 移除。"
  - "只有跨会话个性化值得承担概率误差、延迟和治理成本时才应引入 Mem0；可恢复执行状态交给 LangGraph checkpointer，权威事实仍放普通数据库。"
faq:
  - q: "Mem0 OSS v3 的核心变化是什么？"
    a: "自动提取改为一次 LLM 调用的 ADD-only 流程，不再由第二次调用决定 UPDATE 或 DELETE。检索以语义搜索为基础，再按可用情况融合 BM25 与实体链接信号；search 与 get_all 的用户标识需要放入 filters。"
...
4. **检索管道**：先取得语义候选，再按可用情况融合关键词与实体信号。
...
这些信号服务于候选融合与排序。BM25 和实体匹配主要提升已有语义候选，而不是完全独立的召回通道；相关依赖不可用时，语义搜索仍可工作。[官方迁移指南](https://docs.mem0.ai/migration/oss-v2-to-v3)

**2026-09-24 更正**：核对 `mem0ai==2.2.0` 发布源码后，移除了原文对 OSS 内建时间排序的描述。该版本的 `add(timestamp=...)` 与 `search(reference_date=...)` 会拒绝非空参数，源码将它们标为 Platform 专属。记忆中的时间表达、元数据与应用侧新旧事实归并，不能据此等同于 OSS 内建的时间检索能力。[2.2.0 发布源码](https://github.com/mem0ai/mem0/blob/47a69e1e72dc562b6fdd49a9ef892229afc7508a/mem0/memory/main.py)
...
版本核对（2026-09-24）：下例通过语法检查，调用参数与 [`mem0ai==2.2.0`](https://pypi.org/project/mem0ai/2.2.0/) 的发布源码一致。这里的 v3 指记忆算法，不是 Python 包的主版本号；本次未执行模型、嵌入或存储调用，因此不构成检索质量或完整运行验证。
...
7. [Mem0 2.2.0 发布源码：OSS 参数与时间能力边界](https://github.com/mem0ai/mem0/blob/47a69e1e72dc562b6fdd49a9ef892229afc7508a/mem0/memory/main.py)
8. [mem0ai 2.2.0 Python 包](https://pypi.org/project/mem0ai/2.2.0/)
