...
下面用一组虚构数据展示删除请求如何影响派生记忆。切换要删除的来源，比较哪些内容会被移除，哪些需要重新计算，以及哪些副本仍无法确认。

{{< interactive kind="memory-lineage" id="memory-lineage" spec="memory-lineage-v1" >}}

注意最后一站：无论删除请求覆盖多少来源，未记录 lineage 的聊天摘要、模型提供商缓存与备份都停在“无法证明已抹除”。已失效 ≠ 已验证的物理删除——这正是撤销必须从写入时设计的原因。
