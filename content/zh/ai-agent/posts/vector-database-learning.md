...
tldr:
  - "向量数据库通过数学向量表示数据，相比传统数据库更优化处理非结构化数据和执行高效的相似性搜索。"
  - "向量数据库为AI提供了记忆能力，使AI模型能够高效存储、管理和检索大规模高维数据，支持推荐系统和图像识别等应用。"
  - "近似最近邻搜索算法克服高维空间的维度诅咒问题，通过降维和索引技术实现大规模数据集的快速查询和实时学习。"
---
...
下面这张二维图解把公式变成可以动手验证的实验：拖动或输入坐标、缩放 B 的长度、旋转 B，看点积、夹角和余弦各自怎么变。

{{< interactive kind="vector-cosine" id="cosine-figure" spec="vector-cosine-v1" >}}

试着先只改变 B 的长度：余弦保持不变，因为余弦只看方向；再旋转 B：长度不变，夹角和余弦却随之改变——这正是“只衡量方向上的相似度、不考虑大小”的含义。注意零向量预设：没有方向时，夹角与余弦是未定义的，而不是 0。真实的向量嵌入存在于高维空间，这张二维图只演示几何关系，不能预测任何语义检索效果。
