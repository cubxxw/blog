...
可以用一个确定性的同步推演把“最窄处”变得可操作：生成→审核→交付三段串行，每一拍先到一批固定的新需求，随后按交付→审核→生成的顺序各处理一次。

{{< interactive kind="flow-bottleneck" id="line-figure" spec="flow-bottleneck-v1" >}}

把生成环节的产能从 6 提到 10，你会看到生成前的队列变短，交付速率却停在审核的每拍 3 件——新增产能只是变成了审核前的在制。把审核也提速，瓶颈才迁到交付。这是固定输入的示意推演：真实系统还会反应，需求、质量和瓶颈本身都会移动，所以它只用来建立直觉，不能替代上一段说的持续观察。
