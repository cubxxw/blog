...
3. **易于版本控制和回滚**：Helm charts易于版本控制，你可以追踪到每次部署的变化，并在需要时轻松地回滚到旧版本。
...
![544991a1-6499-4de0-acef-bb43af85fdb3](http://sm.cubxxw.com/sm202311261925321.webp)



集成 Go 语言和其他的一些工具：

1. **GitHub Actions 集成**：在流程图中的各个阶段，如“代码审核”、“漏洞扫描”、“代码分析”等，可以通过 GitHub Actions 自动化执行。也就是说，每个步骤可以被配置为 GitHub Actions 的一个工作流，从而实现自动化和流程控制。
2. **Go 语言相关工具**：
...
![chatp](http://sm.cubxxw.com/sm202311261938086.png)
...
![50629a41-bded-4f36-abfe-b3dfadd7c9d3](http://sm.cubxxw.com/sm202311261945127.webp)
...
![gitops-deployment-replicaset](http://sm.cubxxw.com/sm202311262010756.png)
...
Kubernetes中的服务路由是完全基于标签的。举例来说，一个标记为“blue”的Service只会将流量路由到带有“blue”标签的Pods，而一个标记为“green”的Service则只路由到带有“green”标签的Pods。这种设计允许简单而直观的流量分配和管理。

![ingree-istio](http://sm.cubxxw.com/sm202311262110507.png)
...
![istio-ingress](http://sm.cubxxw.com/sm202311262115919.png)
...
2. **Kubernetes Service 的局限性**：
   ...
   + 因此在进行版本切换时，一些现有的连接可能仍然指向旧版本的Pod，这对于需要立即切换和快速回滚的应用不是最佳选择。
...
![deployment-blue-green](http://sm.cubxxw.com/sm202311270932090.png)
...
渐进式部署是金丝雀部署的一个完全的自动化版本。渐进式交互不会在扩大金丝雀部署之前等一个固定的时间段，它靠的是持续监测 Pod 的健康状态，并且不断扩大规模，直到完全扩展。
