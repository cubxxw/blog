...
使用 Hugo Modules 前，请安装 Git，以及满足所用 Hugo 和依赖模块要求的 Go 版本。[Hugo Modules 使用要求](https://gohugo.io/hugo-modules/use-modules/)

在 Netlify 中，通过 `GO_VERSION` 或构建基础目录内的 `.go-version` 固定已验证的 Go 版本，并与本地环境保持一致；同时存在时，`.go-version` 优先。本教程的构建基础目录就是仓库根目录。[Netlify Go 配置规则](https://docs.netlify.com/build/configure-builds/manage-dependencies/#go)
...
本节查找表及随后 `_default/baseof.html` 示例以 [Hugo 0.145.0 的模板查找实现](https://github.com/gohugoio/hugo/blob/v0.145.0/output/layouts/layout.go#L223-L298)为参照。Hugo 0.146.0 引入了新的模板系统；升级时请按[官方迁移说明](https://gohugo.io/templates/new-templatesystem-overview/)检查目录和查找规则，并验证实际命中的模板。表中每个独立代码片段代表一个候选路径，按箭头顺序查找。
...
- [Hugo Modules：使用要求](https://gohugo.io/hugo-modules/use-modules/)
- [Netlify：Go 版本配置规则](https://docs.netlify.com/build/configure-builds/manage-dependencies/#go)
- [Hugo 0.145.0：模板查找实现](https://github.com/gohugoio/hugo/blob/v0.145.0/output/layouts/layout.go#L223-L298)
- [Hugo 0.146.0：新模板系统迁移说明](https://gohugo.io/templates/new-templatesystem-overview/)
