...
tldr:
  - "go:generate命令自动生成代码以减少样板文件，通过magic comment指定生成器实现高度灵活的代码生成工作流"
  - "go:build指令根据操作系统、架构或自定义标签控制文件编译，支持复杂条件组合实现多平台差异化代码管理"
  - "go:embed在Go1.16中引入，允许直接嵌入静态文件到二进制文件中，简化资源分发并支持处理单文件或整个目录"
---
...
2. **集成到构建流程中**：
    - 在实际的开发流程中，"Go Generate" 命令通常被集成到自动化构建流程中。也就是说，每当代码库被构建时，相关的生成命令也会被自动执行。
...
1. **复杂的构建条件**：
    ...
    - 例如，你可以组合多个条件，如 `//go:build linux && amd64`，代码就仅在目标平台为 Linux 并且架构为 amd64 时才会编译。
2. **与构建标签的结合使用**：
...
`go:notinheap` 指令是一个编译器指令，它告诉go编译器不能在堆上分配类型。这个指令对于避免某些类型对象的垃圾收集开销特别有用。当你使用这个指令时，它所应用的类型的实例就只能在堆栈或全局变量中分配。
...
- 使用 `go:yeswritebarrierrec` 指令：当你在代码中使用这个指令时，你告诉 Go 编译器在接下来的代码块中启用写屏障记录。也就是说，这段代码中的内存写入操作将被写屏障记录，以帮助垃圾回收器正确跟踪对象引用。
