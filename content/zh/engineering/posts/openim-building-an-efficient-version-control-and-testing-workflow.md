...
tags: ["Blog", "Git", "Testing Workflow", "OpenIM"]
...
tldr:
  - "OpenIM采用main稳定分支、release发布分支的分支策略，确保代码质量与稳定性需严格审查和测试后才能合并。"
  - "release分支PR需遵循规范化提交格式（如fix(release-v3.5): description），并配合手动测试截图验证已知问题修复。"
  - "OpenIM社区通过透明项目管理、自动化测试与人工审核相结合，确保代码库的质量、安全性和可持续发展。"
---
