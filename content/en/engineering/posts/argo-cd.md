...
The cleanest way to keep the four decisions apart is to make each one a dial: toggle autoSync, selfHeal and prune, then press manual sync or history rollback on the same delta:

{{< interactive kind="gitops-reconcile" id="argocd-reconcile" spec="gitops-reconcile-v1" >}}

Watch the two independent readouts: **Sync status** only answers whether Git and the cluster match, while **Health** comes from a separate health assessment that no toggle can fabricate. selfHeal targets cluster-side drift only; with prune off, a Git-deleted resource stays in the cluster and keeps showing in the delta; and with automated sync enabled the history rollback action is refused outright — the four constraints above, one by one.
