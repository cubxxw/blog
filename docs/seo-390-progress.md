...
更新：2026-09-25（上海时间）。目标与各子任务验收见 [交付合同](seo-390-delivery-contract.md)。

## 本轮收尾范围

本轮交付已验证的内容、摘要、规范路径与兼容性修复，以及 A+B1+B2 的完整采集、确定性分析和可信日报链路。性能对照、URL Inspection 和新的搜索意图实验继续保留在各自 issue，不在收尾期间开启新批次。实现、线上运行和搜索结果分别记录；本页不会把本地测试通过写成 Google 收录或点击增长。

| 任务 | 已有证据 | 仍需完成的验收 |
|---|---|---|
| #391 数据口径 | A 的 84 项测试及独立探针通过，冻结历史基线逐字节复现 | 新采集器真实 56 天 query×page 回填、实际窗口和完整性读回 |
| #392 分析与日报 | A+B1+B2 已集成；273 项测试和 14 项实际工作流分支探针通过 | 最终提交 CI、真实 Analyze / proposal-only 运行与三次计划运行 |
| #393 索引与迁移 | sitemap 417 页 HTTP/metadata 核验；UFO 和八个旧路径入口修复读回 | URL Inspection、GSC 分类导出、四条查询参数规则的目标语义 |
| #394 工程教程 | 示例错误修复已上线；Hugo 摘要、模板路径表和版本边界通过局部验证 | 本轮 Hugo 修改线上读回、真实 page-query 实验与重新抓取后的观察 |
| #395 性能 | 历史成本定位；无 JS 字体后备 40 个组合验证通过 | 固定部署、浏览器和节流条件的前后对照；不宣称 LCP 已改善 |
| #396 月度入口 | 确认已有主题导航，保留档案和稳定锚点 | 依据 page-query 选择试点；独立新文章仍需作者终审 |
| #397 项目文章 | LangGraph 原示例无网络执行通过；Mem0 OSS/Platform 边界更正已审 | 本轮修改线上读回、目标页面查询证据和搜索结果观察 |

父工作树已合入并完整保留并行 #389 的 `10e7039d`；B2 导入的 24 个路径与其后续 133 个变更路径无交叉。该基线的构建、Pages、Lighthouse、README 和 E2E 全部通过。本轮联合发布的提交、CI 与真实运行读回持续记录在 [#390 交付回执](https://github.com/cubxxw/blog/issues/390#issuecomment-5813098179)，不得用基线绿灯代替本轮提交验收。

### A+B1+B2 集成验收

Pi/MiMo Pro 完成采集、报告与可信发布接线，父代理逐批复核并保留每次失败反例。最终集成的八组 Node 定向测试 **273/273** 通过；真实 YAML shell 分支独立探针 **14/14** 通过，覆盖持久化失败、模型解释缺失或损坏、报告失效、提案产物缺失及跨午夜发布。五份 workflow 的 actionlint 检查通过；本地未执行全站构建或浏览器下载。

父代理在最后一轮补正两项接线：损坏的可选解释在 aggregate 与 compose 中统一标为 failed/withheld，不改变必需阶段结果；提案发布检查 schema、模式和决策时间，空 gate/context 进入最小失败说明，不能冒充正常零候选。正常提案与降级路径均有实际命令行验证。

新 Snapshot 保存本次精确 GSC/PSI/CrUX 原件及四个必需阶段状态，Git 保存失败也能从 artifact 取回证据。Analyze 的数字由确定性代码生成，模型仅能输出一段有界文字；Autofix 仅发布提案。模型原始执行记录不上传，识别到的错误只保留常量分类。本页的离线通过不证明 Google 端采集、模型 OAuth、真实 Linux 运行或搜索增长。

## 前期研究与验证记录

以下记录按各次验证当时的边界保留；当前发布状态以上方收尾表和最新交付记录为准。
...
性能后续批次应先固定测量条件，再选择一个有证据的改动；本轮收尾不启动新性能实验。保留中文衬线设计、SSR 正文、无 JS 可读性和 reduced-motion；不能用 macOS 自带字体的结果代替缺少这些本地字体的设备表现。
...
- 联合发布的精确提交 CI 与真实运行读回见上方交付回执；三次计划运行和 Google 端观察不能由离线检查替代。
