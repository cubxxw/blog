...
2. **以完整日期/切片替换重叠数据。** 成功空响应、缺失、失败、部分分页、旧格式未知完整性使用不同状态。失败不能覆盖已成功结果；成功空日应能替换旧结果。范围或聚合方式不同的切片不可互相覆盖。
...
| C | [393](https://github.com/cubxxw/blog/issues/393) | 索引导出导入与 URL 分诊工具、逐篇重定向验证 | 主站/子站与 sitemap 分离；无循环、终点语义对应；不泛跳首页 | 全 11 类 GSC 导出、P1 URL Inspection、迁移设置、T+14/T+28 |
